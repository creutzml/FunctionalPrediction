...
#' band <- make_band_FFSCB_z(
#'   x = hat.mu, diag.cov.x = diag(hat.cov.mu), tau = hat.tau,
#'   conf.level = 0.95
#' )
...
.make_band_FFSCB_z <- function(tau, diag.cov, conf.level = 0.95, n_int = 4, n.curves, int.type = "confidence", one.sided = F) {
  ...
  tau_f <- function(t) {
    stats::approx(x = seq(0, 1, len = length(tau)), y = tau, xout = t)$y
  }
  ...
  if (one.sided) {
    alpha.level.comp <- alpha
  } else {
    alpha.level.comp <- alpha / 2
  }
  ...
  if (n_int == 1) { # Case n_int=1 == constant band == Kac-Rice Band
    ...
    myfun1 <- function(c1) {
      stats::pnorm(q = c1, lower.tail = F) + exp(-c1^2 / 2) * tau01 / (2 * pi) - alpha.level.comp
    }
  ...
  for (j in (const_int + 1):n_int) {
    fct_body <- paste0(fct_body, "+ c_v[", j, "]*pmax(t - knots[", j, "],0)")
  }
...
.make_band_FFSCB_t <- function(tau, diag.cov, df, conf.level = 0.95, n_int = 4, one.sided = F, int.type = "confidence", n.curves = NULL) {
  ...
  if (n_int == 1) { # Case n_int=1 == constant band == Kac-Rice Band
    tau01 <- sum(tau_v) * diff(tt)[1] # int_0^1 tau(t) dt
    myfun1 <- function(c1) {
      c(stats::pt(q = c1, lower.tail = FALSE, df = nu) + (tau01 / (2 * pi)) * (1 + c1^2 / nu)^(-nu / 2) - (alpha.level.comp))
    }
    const_band <- stats::uniroot(f = myfun1, interval = c(0, 10), extendInt = "downX")$root
  ...
  for (j in (const_int + 1):n_int) { # j <- const_int+1
    myfunj <- function(cj) {
      ...
      afun_j <- function(t, cj) {
        sqrt(nu * tau_f(t)^2 * (1 + ufun_j(t, cj)^2 / nu) / nup)
      }
      ##
      fn1 <- function(t, cj) {
