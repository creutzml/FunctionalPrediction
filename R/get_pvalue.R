...
get_pval_Ec <- function(x, x0 = NULL, eigen, fpc.cut = NULL, prec = NULL) {
  if (is.null(prec)) {
    prec <- c(10^(-6), 10^(-6), 10000)
  }
  if (is.null(fpc.cut)) {
    fpc.cut <- sum(eigen$values > .Machine$double.eps)
  }
  if (is.null(x0)) {
    x <- x - x0
  } # this works both for "fd" and vector.
  ##
  c.square <- sqrt(eigen$values[1:fpc.cut])
  if (inherits(x, "fd")) {
    coef.square <- fda::inprod(x, eigen$harmonics[1:fpc.cut])^2
  } else {
...
get_pvalue_FFSCB_z <- function(x, x0 = NULL, tau, diag.cov, eval.points = NULL, n_int = 5) {
  ##
  if (is.null(x0)) {
    x0 <- rep(0, times = length(x))
  }
  if (is.null(eval.points)) {
    stop("Please specify 'eval.points' (in [0,1]) at which the pvalue should be computed.")
  }
  ##
  t_grid <- eval.points
  ...
  ##
  myfun <- function(p, t) {
    b <- .make_band_FFSCB_z(tau = tau, diag.cov = diag.cov, conf.level = (1 - p), n_int = n_int)$band
  ...
  for (i in 1:length(t_grid)) {
    p_grid[i] <- stats::optimize(f = function(p) {
      myfun(p = p, t = t_grid[i])
...
get_pvalue_FFSCB_t <- function(x, x0 = NULL, tau, diag.cov, df, eval.points = NULL, n_int = 5) {
  ...
  ##
  myfun <- function(p, t) {
    b <- .make_band_FFSCB_t(tau = tau, diag.cov = diag.cov, df = df, conf.level = (1 - p), n_int = n_int)$band
    b_f <- stats::splinefun(x = seq(0, 1, len = length(tau)), y = b, method = "natural")
    sgn <- sign(x_f(t) - x0_f(t))
    tmp <- x_f(t) - sgn * b_f(t)
    return((tmp - x0_f(t))^2)
