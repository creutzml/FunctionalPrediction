...
predict.concurrent <- function(concurrent_list, interval = "prediction", new_dat = NULL, time_vec = NULL, mse_scalar = "ub", err_str = "t", nu0_hat = "singh_df",...
  ...
  if (!(err_str %in% c("normal", "t"))) {
    stop("Argument err_str (error structure) must be specified", 
         "as 'normal' or 't' \n")
  }
  ...
  if (err_str == "normal" & !(conf_type == "FFSCB.z")) {
    stop("Argument `err_str` must match match argument `type`. E.g.",
  ...
  if (is.null(time_vec)) {
    ...
    stop("Argument `time_vec` is of different length than", 
  ...
  if (is.vector(new_dat)) {
    if (length(new_dat) != n_pred - 1) {
      stop("Argument `new_dat` must provide an observed value for ", 
  ...
  x_mat_inv <- array(
    apply(x_array, 1, FUN = function(x) {solve(t(x)%*%x)}),
