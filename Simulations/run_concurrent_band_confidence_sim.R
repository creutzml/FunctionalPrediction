...
### August 16th, 2023                                             ###
###                                                               ###
###   Updated simulation study. This simulation checks the        ###
### coverage levels of the confidence bands for concurrent        ###
### regression (Creutzinger et al. 2024+).                         ###
#####################################################################
...
for (p in 1:nrow(sim_parms)) {
  ...
  sim_results_temp <- data.frame(
    err_dist = rep(sim_parms$err_dist[p], n_iters*2),
    n = rep(sim_parms$n[p], n_iters*2),
    dfs = rep(sim_parms$dfs[p], n_iters*2),
    # sigs = rep(sim_parms$sigs[p], n_iters*4),
    # method = vector("character", n_iters*4), 
    prediction = vector("numeric", n_iters*2), 
    out = vector("numeric", n_iters*2),
    out1 = vector("numeric", n_iters*2),
    out2 = vector("numeric", n_iters*2),
    out3 = vector("numeric", n_iters*2),
    # out4 = vector("numeric", n_iters*4),
    # out5 = vector("numeric", n_iters*4),
    band_score = vector("numeric", n_iters*2),
    max_band_width = vector("numeric", n_iters*2)
  ...
  for (iter in 1:n_iters) {
    ...
    # Check coverage
    ff_10_out_temp <- (sum(
      true_int_mean < fBands[[1]]$band[,3] |
        true_int_mean > fBands[[1]]$band[,2]
    ) > 0)
    ...
    for (c in 1:3) {
      ...
      ff_10_out_temp_by_int[c] <- (sum(
        true_int_mean[start_id:end_id] < 
          fBands[[1]]$band[start_id:end_id,3] |
          true_int_mean[start_id:end_id] > 
          fBands[[1]]$band[start_id:end_id,2]
      ) > 0)
    }
    
    # Calculate band score
    ff_10_bscore_temp <- band_score(
      new_obs = true_int_mean, 
      band_lower = fBands[[1]]$band[,3],
      band_upper = fBands[[1]]$band[,2], 
      sig_level = 0.1
    )
    
    # Calculate mean band width
    ff_10_bwidth_temp <- max(fBands[[1]]$band[,2] - fBands[[1]]$band[,3], 
                             na.rm = T)
    ...
    # Check coverage
    ff_11_out_temp <- (sum(
      true_int_beta_mean < fBands2[[1]]$band[,3] |
        true_int_beta_mean > fBands2[[1]]$band[,2]
    ) > 0)
    ...
    for (c in 1:3) {
      ...
      ff_11_out_temp_by_int[c] <- (sum(
        true_int_beta_mean[start_id:end_id] < 
          fBands2[[1]]$band[start_id:end_id,3] |
          true_int_beta_mean[start_id:end_id] > 
          fBands2[[1]]$band[start_id:end_id,2]
      ) > 0)
    }
    
    # Calculate band score
    ff_11_bscore_temp <- band_score(
      new_obs = true_int_beta_mean, 
      band_lower = fBands2[[1]]$band[,3],
      band_upper = fBands2[[1]]$band[,2], 
      sig_level = 0.1
    )
    
    # Calculate mean band width
    ff_11_bwidth_temp <- max(fBands2[[1]]$band[,2] - 
                               fBands2[[1]]$band[,3], 
                             na.rm = T)
    #################################################################
    ...
    sim_results_iter <- matrix(
      c(#"Conformal", "Conformal", 
        #"Fast and Fair", "Fast and Fair",
        "c(1,0)", "c(1,1)", #"c(1,0)", "c(1,1)",
        #conf_10_out_temp, conf_11_out_temp, 
        ff_10_out_temp, ff_11_out_temp, 
        ...
        #conf_10_bscore_temp, conf_11_bscore_temp, 
        ff_10_bscore_temp, ff_11_bscore_temp, 
        #conf_10_bwidth_temp, conf_11_bwidth_temp, 
        ff_10_bwidth_temp, ff_11_bwidth_temp), 
      nrow = 2
    )
    ...
    sim_results_temp[s_idx:e_idx, c(4:10)] <- sim_results_iter
